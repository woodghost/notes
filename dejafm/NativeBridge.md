...
这些方法注册在window object下，在console可以直接调用 e.g. window.__dejafashion_data_share 查看

Main code

```javascript
function NativeBridge(protocolHandler) {
//传 protocolHandler arg进来（是一个function）
  var _NB = this,
    global = window,
    //define a empty function, this trick show many times
    emptyFn = function () {
    },
  ...
  function protocol(action, callback) {
    ...
    //开启调试, debug为true simulates circumstance in App
    if (debug && callback) {
      var _data = action.match(/[\w]:\/\/(.*)/);//RegExp match的是什么样的url呢，没太理解这里
